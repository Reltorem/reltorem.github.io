...
- If a device cannot reach a DHCP server, it can assign itself a link-local address. This allows the device to still communicate on the immediate LAN which can be useful while you troubleshoot obtaining a proper IP address. It is not forwarded by routers and so cannot be routed over the internet.

- APIPA uses the address range **169.254.0.0/16** (169.254.0.0 - 169.254.255.255)
- First and last 256 addresses of this block are reserved, partly for legacy vendor compatability reasons who already make use of this address space for their own link-local functionality.
- Therefore, actual assignable range is: **169.254.1.0 - 169.254.254.255**
- Addresses are assigned randomly from this pool.
- ARP request is broadcast to local subnet asking if any device already has the IP, if no reponse is received it is assumed that the randomly assigned IP is free to use.
